...
import { BUDGETS, CAT, fmtMoney } from '@/lib/data';
...
export default function PresupuestosPage() {
  ...
  return (
    <div style={{ padding: '28px 32px 40px', maxWidth: 840 }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-3)', marginTop: 4 }}>
            {lang === 'es' ? 'Mes de Mayo 2026' : 'May 2026'}
          </div>
        </div>
        <button style={{
      ...
      <div className="cd-card" style={{ padding: '22px 24px', marginBottom: 20, position: 'relative', overflow: 'hidden' }}>
        <div style={{
          ...
          background: 'radial-gradient(ellipse at 0% 50%, color-mix(in oklab, var(--cyan) 8%, transparent), transparent 60%)',
          pointerEvents: 'none',
        }} />
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
              ...
              <span className="mono" style={{ fontSize: 14, color: 'var(--text-3)' }}>
                {t.of} {fmtMoney(totalLimit, currency)}
              </span>
            </div>
          </div>
          <div style={{ flex: 1 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {BUDGETS.map(b => {
          ...
          const danger = pct > 85;
          const isHovered = hoverId === b.cat;
          const barColor = over ? 'var(--expense)' : danger ? 'var(--warn)' : c.color;

          return (
            <div
              key={b.cat}
              className="cd-card"
              style={{ padding: '18px 20px', cursor: 'pointer', transition: 'border-color 150ms', borderColor: isHovered ? 'var(--border-strong)' : 'var(--border)' }}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                      <span className="mono" style={{ fontSize: 14, color: over ? 'var(--expense)' : 'var(--text)', fontWeight: 600 }}>
                        {fmtMoney(b.spent, currency)}
                      </span>
                      <span className="mono" style={{ fontSize: 12, color: 'var(--text-3)' }}>
                        {t.of} {fmtMoney(b.limit, currency)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11.5, color: over ? 'var(--expense)' : danger ? 'var(--warn)' : 'var(--text-3)', fontWeight: 500 }}>
                      {over
                        ? `${fmtMoney(b.spent - b.limit, currency)} ${t.over}`
                        : `${fmtMoney(b.limit - b.spent, currency)} ${t.left}`}
                    </span>
                    <span className="mono" style={{ fontSize: 12, fontWeight: 600, color: over ? 'var(--expense)' : danger ? 'var(--warn)' : 'var(--text-3)' }}>
                      {Math.round(pct)}%
                    </span>
                  </div>
                </div>
